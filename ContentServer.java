...
public void start() {
  ...
  // Reads the data from the local file and converts it into a WeatherData object
  private WeatherData readDataFromFile() throws IOException {
    ...
    while ((line = reader.readLine()) != null) {
        ...
        if (parts.length == 2) {
            ...
            // Map key-value pairs to the WeatherData fields (e.g., id, name, air_temp)
            data.setField(key, value);  // Assume this method exists in WeatherData
        }
    }
    reader.close();
    return data;
